...
export default function editCell(editProps,editFun ){
  ...
  const doView=editProps.doView||null;
  if(tagType==='input'){
    return  <Input
      defaultValue={value}
    ...
    >
      {
        editCellOptions[keyWord].map((item, key) => <Select.Option value={item.value} key={key}>{item.name || item.value}</Select.Option>)
      }
    </Select>
    ...
    return  <div>
              ...
              <Button
                type="primary"
                icon="search"
                onClick={searchCellBtn||''
                }/>
            </div>
  }else if(tagType==='p'){
     return <p>
         <a href="javascript:;" onClick={doView||''}>查看</a>
       </p>
  }
}
